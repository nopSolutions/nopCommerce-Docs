...
- **IMultiFactorAuthenticationMethod**. মাল্টি-ফ্যাক্টর প্রমাণীকরণ মেথড যেমন *GoogleAuthenticator* ইত্যাদি তৈরির জন্য ব্যবহৃত হয়।
     >[!NOTE]
...
- আপনার কর্ম মেথডের জন্য নিম্নলিখিত বৈশিষ্ট্যগুলি ব্যবহার করুন:
    ...
    > [!TIP]
    > আপনি এই বৈশিষ্ট্যগুলিকে সরাসরি নিয়ামকের সাথে যুক্ত করতে পারেন। এই ক্ষেত্রে, তাদের সাথে প্রতিটি মেথড ট্যাগ করার প্রয়োজন নেই।
...
একবার আপনি আপনার প্লাগইন ইন্সটল করে কনফিগারেশন মেথড যোগ করলে আপনি **প্রশাসন → কনফিগারেশন → লোকাল প্লাগইন** এর অধীনে কনফিগার করার জন্য একটি লিঙ্ক পাবেন।
...
## "InstallAsync", "UninstallAsync" এবং "UpdateAsync" মেথডগুলি পরিচালনা করা
...
১. **InstallAsync**. প্লাগইন ইনস্টলেশনের সময় এই মেথডটি চালু হয়। আপনি এখানে যেকোনো সেটিংস শুরু করতে পারেন, নতুন লোকাল রিসোর্স সন্নিবেশ করতে পারেন, অথবা কিছু নতুন ডাটাবেস টেবিল তৈরি করতে পারেন (প্রয়োজন হলে)।

২. **UninstallAsync**. প্লাগইন আনইনস্টল করার সময় এই মেথডটি চালু হয়।

৩. **UpdateAsync**. প্লাগইন আপডেটের সময় এই মেথডটি চালু করা হবে (যখন এর সংস্করণ `plugin.json` ফাইলে পরিবর্তিত হয়)।

> [!IMPORTANT]
> আপনি যদি এই মেথডগুলির মধ্যে একটিকে ওভাররাইড করেন তবে এর ভিত্তি বাস্তবায়ন গোপন করবেন না।

উদাহরণস্বরূপ, ওভাররাইড করা `InstallAsync` মেথডে নিম্নলিখিত মেথড কল অন্তর্ভুক্ত করা উচিত: *`base.Install()`*। *PayPalStandard* প্লাগইন এর `InstallAsync` মেথডটি নিচের কোডের মত দেখায়
